...
public class Server {
  ...
  public Task Login(bool fresh = false) {
    lock (loginLock) {
      return login is { IsCompleted: false } ? login : login = Task.Run(async () => {
        try {
          if (fresh) throw new Exception("New authentication required");
          OAuth = await Twitchy.Refresh<Authorized>(OAuth?.Tokens.refresh_token ?? throw new Exception("No refresh token available"));
        } catch { OAuth = await Twitchy.Authorize<Authorized>(); }
      });
    }
  }
