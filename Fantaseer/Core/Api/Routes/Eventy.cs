...
public class Eventy() : Route("api/events") {
  ...
  public async Task<T> Publish<T>(IEnumerable<Options> opts, int tries = 3) {
    for (int i = 1; ; i++) {
      try { return await Response<T>(Opts(opts)); } catch (Exception ex) {
